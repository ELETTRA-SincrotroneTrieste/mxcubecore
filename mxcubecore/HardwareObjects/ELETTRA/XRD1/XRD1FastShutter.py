...
class XRD1FastShutter(AbstractShutter):

    def __init__(self, *args, **kwargs):

        super(XRD1FastShutter, self).__init__(*args, **kwargs)
    ...
    def init(self):
        ...
        super(XRD1FastShutter, self).init()
        self.ch_shutter_state = self.get_channel_object("fast_shutter_state",
                                                        optional=False)
    ...
    def _set_value(self, value):
        ...
        try:
            if self.get_value() != value:
                self.ch_shutter_state.set_value(tango_value)
                time.sleep(0.2)
                with gevent.Timeout(
            ...
            err_msg = f"Failed to change the state of the \"{self.username}\" setting" \
                      f" the attribute \"{self.ch_shutter_state.attribute_name}\" of" \
                      f" the tango device \"{self.ch_shutter_state.device_name}\" to" \
                      f" \"{tango_value}\""
