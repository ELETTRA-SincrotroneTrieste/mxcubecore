...
class XRD1SampleView(AbstractSampleView):
    def __init__(self, name):

        super(XRD1SampleView, self).__init__(name)
        self._shapes = {}

    def init(self):

        super(XRD1SampleView, self).init()
        ...
        # TODO evaluate connection with "streamSizeChanged" (verify that affect the
        #  position of the click)
        #  e.g. self.connect(self._camera, "streamSizeChanged", self.update_beam_position_on_screen)

        self.update_beam_position_on_screen()
    ...
    def update_beam_position_on_screen(self):

        if int(self._camera.get_stream_size()[0]) == 0:
            stream_width = self._camera.get_width()
            stream_height = self._camera.get_height()
        else:
            stream_width, stream_height, _ = self._camera.get_stream_size()
        HWR.beamline.beam.set_beam_position_on_screen(
            (float(stream_width) / 2.0 + HWR.beamline.diffractometer.beam_center_x,
             float(stream_height) / 2.0 + HWR.beamline.diffractometer.beam_center_y)
        )
