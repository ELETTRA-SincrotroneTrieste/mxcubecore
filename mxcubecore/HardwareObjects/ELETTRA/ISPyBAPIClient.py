...
class ISPyBAPIClient(HardwareObject):
    ...
    def init(self):
        ...
        self.SqlAlchemySession = sessionmaker(
            create_engine(
                ispyb.sqlalchemy.url(self.ispyb_config), connect_args={"use_pure": True}
            )
        )
        try:
    ...
    def get_todays_session(self, prop_info, create_session=False):
        ...
        for session in sessions:
            ...
            try:
                ...
                self.log.exception(
                    "Fail to create struct_time object from " "startDate"
                )
            else:
                try:
                    end_struct = time.strptime(end_date, "%Y-%m-%d %H:%M:%S")
                except ValueError:
                    self.log.exception("Fail to create struct_time object from endDate")
                else:
        ...
        if today_sessions is None:
            ...
            self.log.debug(
                f"Getting local contact for session with ID: " f"{session_id}"
            )
    ...
    def echo(self):
        ...
        if not self.ispyb_api_conn:
            err_msg = (
                f"Error in lims echo: Could not connect to ISPyB DB using "
    ...
    def extract_data_collection_group_from_mx_collection(self, mx_collection: dict):
        ...
        if mx_collection.get("collection_grp_id"):
            data_collection_group["dataCollectionGroupId"] = mx_collection[
                "collection_grp_id"
            ]
    ...
    def extract_data_collection_from_mx_collection(self, mx_collection: dict) -> dict:
        ...
        if mx_collection.get("collection_grp_id"):
            data_collection["dataCollectionGroupId"] = mx_collection[
                "collection_grp_id"
            ]
        if mx_collection.get("pos_id"):
        ...
        if mx_collection.get("sample_reference"):
            data_collection["BLSAMPLEID"] = mx_collection["sample_reference"][
                "blSampleId"
            ]
        if mx_collection.get("fileinfo"):
            if mx_collection["fileinfo"].get("directory"):
                data_collection["imageDirectory"] = mx_collection["fileinfo"][
                    "directory"
                ]
            ...
            if mx_collection["fileinfo"].get("run_number"):
                data_collection["dataCollectionNumber"] = mx_collection["fileinfo"][
                    "run_number"
                ]
            if mx_collection["fileinfo"].get("template"):
                data_collection["fileTemplate"] = mx_collection["fileinfo"]["template"]
            if mx_collection["fileinfo"].get("xtalSnapshotsPaths"):
                # ipsyb allow to store only 4 xtal snapshots paths
                for i, xtal_snapshot_path in enumerate(
                    mx_collection["fileinfo"]["xtalSnapshotsPaths"][:4]
                ):
                    data_collection[
                        "xtalSnapshotFullPath%s" % (i + 1)
                    ] = xtal_snapshot_path
        ...
        if mx_collection.get("oscillation_sequence"):
            if mx_collection.get("oscillation_sequence")[0].get("number_of_images"):
                data_collection["numberOfImages"] = mx_collection[
                    "oscillation_sequence"
                ][0]["number_of_images"]
            if mx_collection.get("oscillation_sequence")[0].get("exposure_time"):
                data_collection["exposureTime"] = mx_collection["oscillation_sequence"][
                    0
                ]["exposure_time"]
            if mx_collection.get("oscillation_sequence")[0].get("start_image_number"):
                data_collection["startImageNumber"] = mx_collection[
                    "oscillation_sequence"
                ][0]["start_image_number"]
            if mx_collection.get("oscillation_sequence")[0].get("start"):
                data_collection["axisStart"] = mx_collection["oscillation_sequence"][0][
                    "start"
                ]
            if mx_collection.get("oscillation_sequence")[0].get("range"):
                data_collection["axisRange"] = mx_collection["oscillation_sequence"][0][
                    "range"
                ]
            if mx_collection.get("oscillation_sequence")[0].get("end"):
                data_collection["axisEnd"] = mx_collection["oscillation_sequence"][0][
                    "end"
                ]
            if mx_collection.get("oscillation_sequence")[0].get("overlap"):
                data_collection["overlap"] = mx_collection["oscillation_sequence"][0][
                    "overlap"
                ]
        ...
        if mx_collection.get("actualCenteringPosition"):
            data_collection["actualCenteringPosition"] = mx_collection[
    ...
    def get_proposal(self, proposal_code: str, proposal_number: int):
        ...
        try:
            with self.SqlAlchemySession() as sql_session:
                ...
                db_blsession_ls: list[BLSession] = sql_session.query(BLSession).filter(
                    BLSession.proposalId == db_proposal.proposalId
                )
            ...
            self.log.exception(
                f"Error occurred retrieving proposal information" " by code and number"
            )
            prop_info = model.ProposalInfo(
                status=model.Status(code="error", msg=str(e))
            )
    ...
    def get_proposals_by_user(self, user_name):
        ...
        try:
            with self.SqlAlchemySession() as sql_session:
                db_person: Person = (
                    sql_session.query(Person).filter(Person.login == user_name).first()
                )
            ...
            props_info_ls = [
                prop_info.dict(exclude_unset=True) for prop_info in props_info_ls
            ]
        except SQLAlchemyError as e:
            self.log.exception(
                "Error occurred retrieving proposal information" " by code and number"
            )
            props_info_ls = [
                model.ProposalInfo(status=model.Status(code="error", msg=str(e)))
            ]
        return props_info_ls

    @hwo_header_log
    def get_protein(
        self, proposal_id: int, acronym: str, sql_session: DBSession = None
    ):
        """
        ...
        try:
            with self.SqlAlchemySession() as sql_session:
                ...
                query = (
                    ...
                    .join(Protein)
                    .join(
                        DataCollection, BLSample.blSampleId == DataCollection.BLSAMPLEID
                    )
                    .filter(Protein.proposalId == proposal_id)
                )
                if session_id:
                ...
                for db_sample in db_samples:
                    ...
                    sample_info = {
                        ...
                        "containerSampleChangerLocation": db_sample.Container.sampleChangerLocation
                        if db_sample.Container
                        else "0",  # 0 = Manual mount
                    ...
                    if db_sample.DiffractionPlan:
                        sample_info["diffractionPlan"] = {
                            "diffractionPlanId": db_sample.DiffractionPlan.diffractionPlanId,
                            "experimentKind": db_sample.DiffractionPlan.experimentKind,
                            "numberOfPositions": db_sample.DiffractionPlan.numberOfPositions,
                            "observedResolution": db_sample.DiffractionPlan.observedResolution,
                            "preferredBeamDiameter": db_sample.DiffractionPlan.preferredBeamDiameter,
                            "radiationSensitivity": db_sample.DiffractionPlan.radiationSensitivity,
                            "requiredCompleteness": db_sample.DiffractionPlan.requiredCompleteness,
                            "requiredMultiplicity": db_sample.DiffractionPlan.requiredMultiplicity,
                            "requiredResolution": db_sample.DiffractionPlan.requiredResolution,
                        }
            ...
            self.log.exception(
                f"Failed to connect to ISPyB DB getting the "
                f"samples of the proposal with id: {proposal_id}"
            )
            return
        except ispyb.ISPyBException:
            self.log.exception(
                f"Failed to interface with ISPyB DB getting the "
                f"samples of the proposal with id: {proposal_id}"
            )
            return
        except Exception:
            self.log.exception(
                f"Unexpected error occurred getting the samples"
    ...
    def get_session_samples(
        self, session_id, sample_name=None, acronym=None, sql_session: DBSession = None
    ):
        ...
        if acronym is not None:
            db_query = (
                db_query.join(Crystal).join(Protein).filter(Protein.acronym == acronym)
            )
    ...
    def add_manual_session_sample(self, session_id, sample_name, protein_acronym):
        ...
        db_samples = self.get_session_samples(
            session_id, sample_name=sample_name, acronym=protein_acronym
        )
        ...
        if not db_samples:
            ...
            if db_protein is None:
                protein_dict = {"acronym": protein_acronym, "proposalId": proposal_id}
                db_protein = self._insert_protein(protein_dict)
                self.log.info(f"New crystal [acronym:{protein_acronym}]" f" created")
            ...
            self.log.info(
                f"Sample [name:{sample_name} - " f"acronym:{protein_acronym}] created"
            )
        else:
    ...
    def _insert_protein(self, protein_dict: dict, sql_session: DBSession = None):
        ...
        self.log.info(
            f'Record inserted in the ISPyB "protein" table.'
            f"\nRECORD:\n{db_protein.__dict__}"
        )
        return db_protein
    ...
    def _insert_crystal(self, crystal_dict: dict, sql_session: DBSession = None):
        ...
        self.log.info(
            f'Record inserted in the ISPyB "crystal" table.'
            f"\nRECORD:\n{db_crystal.__dict__}"
        )
        return db_crystal
    ...
    def _insert_sample(self, sample_dict: dict, sql_session: DBSession = None):
        ...
        self.log.info(
            f'Record inserted in the ISPyB "sample" table.'
            f"\nRECORD:\n{db_samp.__dict__}"
        )
        return db_samp
    ...
    def _insert_grid_info(self, grid_info_dict: dict, sql_session: DBSession = None):
        ...
        self.log.info(
            f'Record inserted in the ISPyB "gridinfo" table\n'
            f"RECORD:\n{db_grid_info.__dict__}"
        )
        return db_grid_info

    @hwo_header_log
    def _insert_motor_positions(
        self, motor_pos_dict: dict, sql_session: DBSession = None
    ):
        """
        ...
        self.log.info(
            f'Record inserted in the ISPyB "motorposition" table.\n'
    ...
    def _insert_position(self, pos_dict: dict, sql_session: DBSession = None):
        ...
        self.log.info(
            f'Record inserted in the ISPyB "position" table.'
            f"\nRECORD:\n{db_pos.__dict__}"
        )
        return db_pos
    ...
    def _insert_data_collection_group(self, dcg_dict, sql_session: DBSession = None):
        ...
        self.log.info(
            f'Record inserted in the ISPyB "datacollectiongroup" '
            f"table.\nRECORD:\n{db_dcg.__dict__}"
        )
        return db_dcg
    ...
    def _insert_data_collection(self, dc_dict: dict, sql_session: DBSession = None):
        ...
        self.log.info(
            f'Record inserted in the ISPyB "datacollection" table.\n'
            f"RECORD:\n{db_data_collection.__dict__}"
        )
        return db_data_collection

    @hwo_header_log
    def _update_grid_info(
        self, grid_id: int, grid_info_dict: dict, sql_session: DBSession = None
    ):
        if not sql_session:
            sql_session = next(self.get_db_session())
        db_grid_info = (
            sql_session.query(GridInfo).filter(GridInfo.gridInfoId == grid_id).one()
        )
        ...
        sql_session.refresh(db_grid_info)
        self.log.info(
            f'Record updated in the ISPyB "gridinfo" table.\n'
            f"RECORD [id {grid_id}]: \n{grid_info_dict}"
        )
        return db_grid_info

    @hwo_header_log
    def _update_motor_positions(
        self, motor_pos_id: int, motor_pos_dict: dict, sql_session: DBSession = None
    ):

        if not sql_session:
            sql_session = next(self.get_db_session())
        db_motor_pos = (
            sql_session.query(MotorPosition)
        ...
        sql_session.refresh(db_motor_pos)
        self.log.info(
            f'Record updated in the ISPyB "motorposition" table.\n'
    ...
    def _update_position(
        self, pos_id: int, pos_dict: dict, sql_session: DBSession = None
    ):

        if not sql_session:
            sql_session = next(self.get_db_session())
        db_pos = sql_session.query(Position).filter(Position.positionId == pos_id).one()
        ...
        sql_session.refresh(db_pos)
        self.log.info(
            f'Record updated in the ISPyB "position" table.\n'
            f"RECORD [id {pos_id}]: \n{db_pos.__dict__}"
        )
        return db_pos

    @hwo_header_log
    def _update_data_collection_group(
        self, dcg_id: int, dcg_dict: dict, sql_session: DBSession = None
    ):
        ...
        sql_session.refresh(db_dcg)
        self.log.info(
            f'Record updated in the ISPyB "datacollectiongroup" table.\n'
            f"RECORD [id {dcg_id}]: "
            f"\n{db_dcg.__dict__}"
        )
        return db_dcg
    ...
    def _update_data_collection(
        self, dc_id: int, dc_dict: dict, sql_session: DBSession = None
    ):

        if not sql_session:
            sql_session = next(self.get_db_session())
        db_dc = (
            sql_session.query(DataCollection)
        ...
        self.log.info(
            f'Record updated in the ISPyB "datacollection" table.\n'
            f"RECORD [id {dc_id}]: \n{db_dc.__dict__}"
        )
        return db_dc
    ...
    def update_data_collection(self, mx_collection, grid_dict=dict({})):
        ...
        with self.SqlAlchemySession() as sql_session:
            ...
            motor_pos_dict = self.extract_motor_position_from_mx_collection(
                mx_collection
            )
            ...
            # DataCollectionGroup
            dcg_dict = self.extract_data_collection_group_from_mx_collection(
                mx_collection
            )
    ...
    def store_data_collection_group_grid(self, dcg_id, grid_dict):
        ...
        dcg_grid_params["dxInMm"] = (
            grid_dict["cell_width"] * 0.001
        )  # This is actually step size not full length
        ...
        self.log.exception(
            f'Failed to insert a record in the ISPyB "gridinfo"'
            f" table.\nRECORD:\n{dcg_grid_params}"
        )
        return grid_info_id
