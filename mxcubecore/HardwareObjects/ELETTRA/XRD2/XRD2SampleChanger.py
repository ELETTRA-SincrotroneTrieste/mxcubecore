...
class XRD2SampleChanger(SampleChanger):
    ...
    def _update_state(self, sc_status=None):
        ...
        self.log.info(f"Sample changer current state: {SampleChangerState.tostring(state)}")
    ...
    def _wait_task(self, timeout: int = 300, waiting_state_list: list = None):
        with Timeout(
            timeout,
            Exception("Timeout waiting for sample changer task finish (executer \"OFF\")")
        ):
            sleep(3)
            while True:
                try:
                    if self.ch_state.get_value() == PyTango.DevState.OFF:
                        ...
                        break
                    if waiting_state_list and not self.get_state() in waiting_state_list:
                        break
                except PyTango.DevFailed as e:
                    self.log.exception("Tango error occurred waiting for samplechanger \"OFF\"")
    ...
    def load(self, sample_location, wait=False):
        ...
        if self.get_state() not in [SampleChangerState.Ready,
                                    SampleChangerState.Defrost]:
            err_msg = f"Sample can not be loaded! Sample changer is not \"Ready\". Use the sample changer " \
        ...
        if not self.is_ready():
            err_msg = f"Sample loading aborted! Sample changer is expected to be \"Ready\" but it's" \
        ...
        try:
            if self.get_loaded_sample() is None:
                ...
                old_basket_selected, old_sample_selected = self._selected_basket, self._selected_sample
            self._selected_sample = sample
            self._selected_basket = basket
            ...
            if sc_gripper == GripperType.Double:
                unloaded_sample_obj = self.get_component_by_address(
                    Container.Pin.get_sample_address(int(old_basket_selected), int(old_sample_selected)))
            ...
            if sc_action_failed:
                ...
                self.log.error(f"Sample loading failed! {err_msg} - StepByStepLog: {step_by_step})")
                loaded = has_been_loaded = False
                ...
                sample_obj._set_loaded(loaded, has_been_loaded)
                self.log.info(f"Sample loaded successfully! Sample {sample_obj.get_coords()} has been mounted")
                status_RA = 'SUCCESS'
    ...
    def unload(self, sample_location=None, wait=None):
        ...
        if self.get_state() not in [SampleChangerState.Ready,
                                    SampleChangerState.Defrost]:
            err_msg = f"Sample can not be unloaded! Sample changer is not \"Ready\". Use the sample changer " \
                      f" actions to manage it, clicking on the equipment tab."
            self.user_log.error(err_msg)
            raise RuntimeError(err_msg)
        ...
        # Abort if not ready
        if not self.is_ready():
            err_msg = f"Sample unloading aborted! Sample changer is expected to be \"Ready\" but it's" \
                      f" \"{SampleChangerState.tostring(self.state)}\" instead"
            self.user_log.error(err_msg)
            raise RuntimeError(err_msg)
        ...
        try:
            ...
            if sc_action_failed:
                ...
                step_by_step = self.ch_step_by_step_log.get_value()
                self.log.error(f"Sample unloading failed! {err_msg} - StepByStepLog: {step_by_step})")
            else:
                sample_obj = self.get_component_by_address(
                ...
                statusRA = 'SUCCESS'
                self.log.debug(f"Sample unloaded successfully! Sample {sample_obj.get_coords()} has been mounted")
    ...
    def get_loaded_sample(self):
        ...
        if -1 in [self._selected_basket, self._selected_sample]:
            err_msg = f"Pin detected but the sample address can not be retrieved from the executer " \
            ...
            self.log.error(f"{err_msg}. Please assign manually the location of the mounted sample")
            #raise RuntimeError(err_msg)
        else:
    ...
    def _do_unpark(self):
        ...
        if self.get_state() is SampleChangerState.Parked:
            ...
            err_msg = "Unparking aborted! Sample changer is not Parked (Current state: %s)"
            self.user_log.error(err_msg, SampleChangerState.tostring(self.get_state()))
    ...
    def reset(self):
        ...
        if self.ch_script_failed.get_value():
            self.log.info("--> SampleChangerElettra_XRD2 _doReset (re)setting samplechanger self.ch_script_failed.get_value() to FALSE")
    ...
    def wait_states(self, states_list: list, timeout=None):
        ...
        with Timeout(
            timeout,
            RuntimeError(f"Timeout waiting one of these states: {states_list}")
        ):
    ...
    def wait_defrost(self, operation: str = 'operation'):
        try:
            self.user_log.warning(f"Sample changer is DEFROSTING, the {operation} will continue after it, please wait")
            self.wait_states([SampleChangerState.Cooldown, SampleChangerState.Ready], timeout=self.DEFROST_DURATION)
            self.user_log.warning(f"DEFROSTING finished, sample changer will go on with the {operation} now!")
        except TimeoutError:
            raise RuntimeError(f"Timeout error occurred waiting sample changer defrosting in {self.operation}")
